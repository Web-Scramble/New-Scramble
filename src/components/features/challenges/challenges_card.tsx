...
import AddParticipantsModal from "../home/add_participant_modal";
import ShareChallengeModal from "../home/share_modal";
import { CommentModal } from "../home/comment_modal";
import { CommentItems } from "../home/comment_items";
import ChallengeUsers from "./challenge_participants";
import { ParticipantsIcon, ReviewersIcon } from "@/assets/icon";
import ChallengeJudges from "./challenge_judges";
...
const AvatarGroup = ({ users, count, onAddClick }: AvatarGroupProps) => {
  return (
    <div className="flex items-center">
      <div className="flex -space-x-3">
        {users.slice(0, 2).map((user, index) => (
          <Avatar key={index} className="border-2 border-white w-8 h-8">
            <AvatarImage
              src={user.profile_picture || "images"}
              alt={user.username}
            />
      ...
      {count > 0 && (
        <span className=" relative -left-3 text-xs text-blue-500 bg-blue-50 w-9 h-9 border-4 border-white rounded-full font-medium flex items-center">
          +{count}
        </span>
      )}
      <Button
        variant="outline"
        size="sm"
        className="rounded-full w-7 h-7 p-0 border-dashed border-blue-300"
        onClick={onAddClick}
      >
      ...
      <Card className=" w-[60%] shadow-none rounded-lg overflow-hidden border-none">
        ...
        <CardContent className="p-4">
          ...
          <div className="flex items-center mb-3">
            <h2 className="text-lg font-semibold text-gray-500 mr-3">
              {title}
            </h2>
            <Badge className="bg-green-100 text-green-600 hover:bg-green-100 px-3">
              {status}
            </Badge>
          </div>
          ...
          <div className="grid grid-cols-12 gap-3 mb-6">
            ...
            <div className="col-span-3 flex flex-col gap-3">
              {media.files.map((file, index) => (
                <div
                  key={index}
                  className="border-2 border-blue-300 rounded-lg p-4 flex flex-col items-center justify-center h-40"
                >
          ...
          <div className="flex flex-wrap justify-between items-center border-t border-b py-4">
            <div className="flex gap-6">
              <div>
                <p className="text-sm text-left font-bold text-gray-500 mb-2">
                  Reviewers
                </p>
                <AvatarGroup
            ...
            <div>
              ...
              <div className="flex items-center">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="ml-2 rounded-full w-7 h-7 p-0 border-dashed border-blue-300"
                >
                  <Plus className="h-4 w-4 text-blue-300" />
                </Button>
              </div>
        ...
        <CardFooter className="px-4 py-3 flex justify-between">
          <div className="flex items-center gap-6">
            <Button
              variant="ghost"
              className={`flex items-center gap-1 p-0 h-auto ${isOpen}? text-[#2E77FA] : text-[#989CA7]`}
              onClick={openComment}
            >
              <Comments />
              {isOpen ? (
                <span className="text-[#2E77FA]">15 Comments</span>
              ) : (
            ...
            >
              <ParticipantsIcon />
              {isUsers ? (
                <span className="text-[#2E77FA]">56 Participants</span>
              ) : (
                <span className="text-gray-500">56</span>
              )}
            </Button>
            ...
            >
              <ReviewersIcon />
              {isReviewer ? (
                <span className="text-[#2E77FA]">56 Revewers</span>
              ) : (
                <span className="text-gray-500">56</span>
              )}
            </Button>
        ...
        <CommentItems isOpen={isOpen} onClose={() => setIsOpen(false)} />
        <ChallengeUsers isOpen={isUsers} onClose={() => setIsUsers(false)} />
        <ChallengeJudges
          isOpen={isReviewer}
          onClose={() => setIsReviewer(false)}
        />
      </Card>
